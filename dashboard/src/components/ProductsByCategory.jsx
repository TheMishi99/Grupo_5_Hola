  ...
  const getProducts = async () => {
    try {
      ...
      const allProducts = await Promise.all(products.map(async (product) => {
        const responseProduct = await fetch("http://localhost:5000/api/products/"+product.id);
    ...
    <div>
      <ul className="container-fluid">
        {filteredProducts.map((product, index) => {
          return (
          <li className="card" style={{width: "18rem"}}key={index + product}>
            <img src={product.image_url} className="card-img-top .h-100" alt={product.name}/>
